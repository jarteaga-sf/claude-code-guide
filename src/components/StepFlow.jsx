...
export default function StepFlow({ stepFlow }) {
  ...
  if (!hasPhases) {
    return (
      <div className="my-8 bg-[var(--color-bg-white)] border border-[var(--color-border)] rounded-lg shadow-[var(--shadow-card)] p-5 space-y-3">
  ...
  return (
    <div className="my-8 space-y-4">
      {stepFlow.phases.map((phase, pi) => (
        <div key={pi} className="bg-[var(--color-bg-white)] border border-[var(--color-border)] rounded-lg shadow-[var(--shadow-card)] p-5">
